...
<head>
    ...
    <link rel="shortcut  icon" type="image/x-icon" href="/template/wap/default/public/images/favicon.ico"
          media="screen"/>
    ...
    <script type="text/javascript">
        var CSS = "/static/css";
        var APPMAIN = 'http://lms.test/';
        var ADMINMAIN = 'http://lms.test/';
            ...
            <ul class="cart-prolist-ul" style="border: 0;">
                <li data-parent-shopid="0">
                    <h2 class="title">
                        ...
                        <div class="cart_ed">
                            <a id="cart_edit{{$item->productSku->product->shop->id}}"
                               data-shopid="{{$item->productSku->product->shop->id}}"
                               onclick="cart_edit(this,{{$item->productSku->product->shop->id}})"
                               style="display: block;">编辑</a>
                            <a id="edit_success{{$item->productSku->product->shop->id}}"
                               onclick="cart_succ(this,{{$item->productSku->product->shop->id}})"
                               style="display: none;">完成</a>
                        </div>
                    </h2>
                </li>
                <li class="cart-list-li" data-shopid="{{$item->productSku->product->shop->id}}">
                    <div class="checkbox" is_check="yes" is_del='no'
                         style="background-image: url('static/picture/cartp1.png')">
                    </div>
                    <div class="product">
                        <div class="pic">
                            <a href="{{ route('products.show',[$item->productSku->product_id]) }}">
                                <img src="/storage/{{$item->productSku->product->image}}" alt="商品图片" width="100%"
                                     height="50px">
                            </a>
                        </div>
                        <div class="info">
                            <p class="info-name">
                                <a href="{{ route('products.show',[$item->productSku->product_id]) }}">{{$item->productSku->product->long_title}}
                                    <span><br/></span></a>
                            </p>
                        </div>
                    </div>
                        ...
                        <div name="edit_num{{$item->productSku->product->shop->id}}" style="display: none;">
								<span class="ui-number">
									<button type="button" class="decrease"
                                            onclick="Cart.changeBar(&#39;-&#39;,{{$item->productSku->id}},this)"
                                            title="减">-</button>
									<input class="num" name="quantity" autocomplete="off"
                                           data-default-num="{{$item->amount}}" value="{{$item->amount}}" min="1"
                                           max="999" max_buy="0" data-cartid="{{$item->id}}"
                                           data-sku_id="{{$item->productSku->id}}"
                                           data-shopid="{{$item->productSku->product->shop->id}}">
